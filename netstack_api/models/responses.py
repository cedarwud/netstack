...
class UEInfoResponse(BaseModel):
    ...
    slice: SliceInfo = Field(
        ...,
        description="目前 Slice 資訊"
    )
...
class UEStatsResponse(BaseModel):
    ...
    slice_switches: int = Field(
        default=0,
        description="Slice 切換次數",
        example=2
    )
    
    last_rtt_test: Optional[datetime] = Field(
        None,
        description="最後 RTT 測試時間"
    )
...
class SliceSwitchResponse(BaseModel):
    ...
    previous_slice: SliceInfo = Field(
        ...,
        description="切換前的 Slice"
    )
    
    new_slice: SliceInfo = Field(
        ...,
        description="切換後的 Slice"
    )
